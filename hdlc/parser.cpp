...
template <typename P> size_t Parser::get_end_pos(P predicate) {
  size_t p = pos;
...
std::shared_ptr<ast::RegWrite> Parser::read_reg_write(SymbolMap &symbol_map) {
  ...
  if (!symbol_map.count(reg_name)) {
    throw ParserError("Register with name " + reg_name + " was not initialized",
                      line, line_pos);
  }
...
Parser::read_assign_stmt(SymbolMap &symbol_map) {
...
Parser::read_ident_list(SymbolMap &symbol_map) {
  ...
  while (true) {
    auto val = std::make_shared<ast::Value>(std::string(read_ident()), nullptr);
